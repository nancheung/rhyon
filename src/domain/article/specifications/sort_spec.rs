...
impl ArticleSortSpec {
    ...
    pub fn to_sort_criteria(&self) -> SortCriteria {
        match self {
            ArticleSortSpec::PublishedAtDesc => {
                SortCriteria::new().desc("published_at")
            },
            ArticleSortSpec::PublishedAtAsc => {
                SortCriteria::new().asc("published_at")
            },
            ArticleSortSpec::TitleAsc => {
                SortCriteria::new().asc("title")
            },
            ArticleSortSpec::TitleDesc => {
                SortCriteria::new().desc("title")
            },
            ArticleSortSpec::CreatedAtDesc => {
                SortCriteria::new().desc("created_at")
            },
            ArticleSortSpec::CreatedAtAsc => {
                SortCriteria::new().asc("created_at")
            },
            ArticleSortSpec::UpdatedAtDesc => {
                SortCriteria::new().desc("updated_at")
            },
            ArticleSortSpec::UpdatedAtAsc => {
                SortCriteria::new().asc("updated_at")
            },
...
impl Default for ArticleSortSpec {
    fn default() -> Self {
        // 默认按发布时间降序排序
        ArticleSortSpec::PublishedAtDesc
    }
